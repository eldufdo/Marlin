...
static float probe_pt(float x, float y, float z_before) {
    ...
    do_blocking_move_to(current_position[X_AXIS], current_position[Y_AXIS], 1);
#ifdef SERVO_ENDSTOPS
            ...
            if (serial_char == '\n') {
                ...
                switch (state) {
                    case 0:
                        ...
                        break;
                    case 1:
                        SERIAL_PROTOCOLPGM("Got G");
                        if (serial_char == '0') {
                            SERIAL_PROTOCOLPGM("Go down");
...
#error "You must have a Z_MIN endstop in order to enable Auto Bed Leveling feature!!! Z_MIN_PIN must point to a valid hardware pin."
                    ...
                    st_synchronize();
                    bool load_bed_level_data = false;
                    if (code_seen('L')) {
                        load_bed_level_data = true;
                        SERIAL_PROTOCOLPGM("Load bed level data from EEPROM\n");
                    }
        
                    bool store_bed_level_data = false;
                    if (code_seen('S')) {
                        load_bed_level_data = false;
                        store_bed_level_data = true;
                        SERIAL_PROTOCOLPGM("Store bed level data to EEPROM\n");
                     }
			...
			reset_bed_level_eeprom();
				...
				continue;
                            ...
                            float measured_z;
                            if (load_bed_level_data == false) {
                                measured_z = probe_pt(xProbe, yProbe, z_before);
                                
                            } else {
                                measured_z = bed_level_eeprom[xCount][yCount];
                            }                       
                            if (store_bed_level_data == true) {
			    	bed_level_eeprom[xCount][yCount] = measured_z;
			    }
                            if (measured_z == 200) {
                                ignore_click = false;
                                break;
